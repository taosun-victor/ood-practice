...
*    Answer: Our current solution is to move the "search()" API fromt "FileSystem" class to           *
*            "Directory" class as a public method, so that we could keep "children" field as PRIVATE. *
*            The API in "FileSystem" class will call node "root"'s search method                      *
*                                                                                                     *
...
//--------------------------- Implementation of Entry and File classes -----------------------
class Entry {
private:
    string name;
public:
...
public:
    ...
    void search(Filter& filter, vector<string>& res){
    	if (this->isFile()){
    		if (filter.isValid((File*)this))
    			res.push_back(this->getName());
    		return;
		}
		
		for (auto m : ((Directory*)this)->children)
			((Directory*)m.second)->search(filter, res);
	}
};
